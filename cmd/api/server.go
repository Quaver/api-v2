...
func initializeRateLimiter(engine *gin.Engine) {
	...
	engine.Use(func(c *gin.Context) {
		...
		if _, canBypassRoute := rateLimitBypassRoutes[c.FullPath()]; canBypassRoute {
...
func initializeRoutes(engine *gin.Engine) {
	...
	engine.GET("/v2/user/:id/relationship", middleware.RequireAuth, handlers.CreateHandler(handlers.GetRelationship))
	...
	engine.GET("/v2/map/:id/mods/:mod_id", handlers.CreateHandler(handlers.GetMapMod))
	...
	engine.GET("/v2/orders/:order_id/:transaction_id", middleware.RequireAuth, handlers.CreateHandler(handlers.GetUserOrder))
