...
CREATE TRIGGER scores_mod_columns_bu
    ...
    FOR EACH ROW
    SET NEW.speed_rate = IF(OLD.mods_migrated = 0,
            CASE
            ...
            END,
            NEW.speed_rate),
        NEW.mirror = IF(OLD.mods_migrated = 0,
            IF((NEW.mods & 2147483648) != 0, 1, 0),
            NEW.mirror),
        NEW.no_slider_velocities = IF(OLD.mods_migrated = 0,
            IF((NEW.mods & 1) != 0, 1, 0),
            NEW.no_slider_velocities),
        NEW.no_long_notes = IF(OLD.mods_migrated = 0,
            IF((NEW.mods & 4194304) != 0, 1, 0),
            NEW.no_long_notes),
        NEW.inverse = IF(OLD.mods_migrated = 0,
            IF((NEW.mods & 536870912) != 0, 1, 0),
            NEW.inverse),
        NEW.full_ln = IF(OLD.mods_migrated = 0,
            IF((NEW.mods & 1073741824) != 0, 1, 0),
            NEW.full_ln),
        NEW.no_miss = IF(OLD.mods_migrated = 0,
            IF((NEW.mods & 17592186044416) != 0, 1, 0),
            NEW.no_miss),
        NEW.mods_migrated = IF(OLD.mods_migrated = 0, 1, NEW.mods_migrated);
